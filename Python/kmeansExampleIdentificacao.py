...
def main():
	...
	num_points = len(tabela)

	# For each of those points how many dimensions do they have?
	# Note: Plotting will only work in two or three dimensions
	dimensions = len(BaseHandle.getEstados('BasePorGenero.txt'))
	...
	points = [
		getPoint(i) for i in range(num_points)
	]
	...
	for i, c in enumerate(clusters):
		cluster = []
		estados = BaseHandle.getEstados('BasePorGenero.txt')
		cluster.append(estados)
		for p in c.points:
			print (" Cluster: ", i, "\t Point :", p)
			linha = [int(i) for i in p.coords]
			linha.insert (0, p.indice)
			cluster.append(linha)
			#cluster.append(p.coords)
...
def getPoint(n):
	...
	'''
	p = Point([float(i) for i in tabela[n]], n)
	return p
